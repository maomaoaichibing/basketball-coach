...
function RadarChart({
  ...
  const keys: (keyof AbilityDimensions)[] = [
    'dribbling',
    'passing',
    'shooting',
    'defending',
  ...
  const values = keys.map((k) => abilities[k] || 5);
  ...
  return (
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} className="mx-auto">
      ...
      {/* 数据区域 */}
      <polygon points={polygonPoints} fill={bgColor} stroke={color} strokeWidth="2.5" />
      ...
      {/* 标签 */}
      {labels.map((label, i) => {
        const p = getPoint(i, 11.2);
        return (
          <g key={`label-g-${i}`}>
            <text
              x={p.x}
              y={p.y - (showValues ? 7 : 0)}
              textAnchor="middle"
            ...
            {showValues && (
              <text
                ...
                textAnchor="middle"
                dominantBaseline="middle"
                className="text-[12px] fill-gray-900 font-bold"
              >
                {values[i]}
              </text>
            )}
...
function AbilityBar({
  ...
  return (
    <div className="flex items-center gap-3">
      <span className="w-12 text-sm text-gray-600 font-medium shrink-0">{label}</span>
      <div className="flex-1 bg-gray-100 rounded-full h-2.5 overflow-hidden">
        <div
          className={`h-full bg-gradient-to-r ${gradients[colorIndex % gradients.length]} rounded-full transition-all duration-700`}
...
export default function GrowthReportDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [abilityViewMode, setAbilityViewMode] = useState<'radar' | 'bars'>('radar');
  ...
  // 计算能力均值和最强/最弱项
  const getAbilityAvg = () => {
    if (!report?.abilities) return 0;
    const vals = Object.values(report.abilities);
    return vals.reduce((a, b) => a + b, 0) / vals.length;
  };

  const getBestAbility = () => {
    if (!report?.abilities) return { key: '-', value: 0 };
    const entries = Object.entries(report.abilities) as [string, number][];
  ...
  const bestAbility = getBestAbility();
  const weakestAbility = getWeakestAbility();
  const abilityAvg = getAbilityAvg();
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-4xl mx-auto px-4 py-6">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden mb-6">
          <div className="bg-gradient-to-r from-orange-500 via-orange-500 to-amber-500 text-white p-6">
            <div className="flex items-center justify-between">
              <div>
                <h2 className="text-xl font-bold mb-3">{report.title}</h2>
              ...
              {report.overallRating && (
                <div className="text-center bg-white/20 rounded-2xl px-6 py-4 backdrop-blur-sm">
                  ...
                  <div className="text-sm text-orange-100 mt-1">综合评分</div>
                </div>
              )}
            </div>
          </div>
        ...
        {report.abilities && (
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 mb-6">
            <div className="grid grid-cols-3 gap-4">
              <div className="flex items-center gap-3 bg-gradient-to-r from-orange-50 to-transparent rounded-lg px-4 py-3">
                <Zap className="w-5 h-5 text-orange-500 shrink-0" />
                <div>
                  <div className="text-xs text-gray-500">能力均值</div>
                  <div className="text-lg font-bold text-gray-900">{abilityAvg.toFixed(1)}</div>
                </div>
              </div>
              <div className="flex items-center gap-3 bg-gradient-to-r from-green-50 to-transparent rounded-lg px-4 py-3">
                <TrendingUp className="w-5 h-5 text-green-500 shrink-0" />
                <div>
                  <div className="text-xs text-gray-500">最强项</div>
                  <div className="text-lg font-bold text-green-600">
                    {abilityLabels[bestAbility.key]} {bestAbility.value}
                  </div>
                </div>
              </div>
              <div className="flex items-center gap-3 bg-gradient-to-r from-blue-50 to-transparent rounded-lg px-4 py-3">
                <Target className="w-5 h-5 text-blue-500 shrink-0" />
                <div>
                  <div className="text-xs text-gray-500">待提升</div>
                  <div className="text-lg font-bold text-blue-600">
                    {abilityLabels[weakestAbility.key]} {weakestAbility.value}
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex items-center gap-1 bg-gray-100 rounded-lg p-0.5">
                <button
                  onClick={() => setAbilityViewMode('radar')}
                  className={`flex items-center gap-1 px-2.5 py-1 text-xs rounded-md transition-colors ${
                    abilityViewMode === 'radar'
                ...
                <button
                  onClick={() => setAbilityViewMode('bars')}
                  className={`flex items-center gap-1 px-2.5 py-1 text-xs rounded-md transition-colors ${
                    abilityViewMode === 'bars'
                      ? 'bg-white shadow text-orange-600'
                      : 'text-gray-500 hover:text-gray-700'
                  }`}
                >
            ...
            {report.abilities && abilityViewMode === 'radar' ? (
              <RadarChart abilities={report.abilities} size={260} />
            ) : (
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="bg-green-50 rounded-xl p-4 text-center">
                ...
                <div className="text-xs text-gray-500 mt-1">出勤率</div>
              </div>
              <div className="bg-orange-50 rounded-xl p-4 text-center">
                <div className="text-3xl font-bold text-orange-600">
                  {report.trainingStats?.avgPerformance || 0}
                </div>
                <div className="text-xs text-gray-500 mt-1">平均表现</div>
              </div>
              <div className="bg-purple-50 rounded-xl p-4 text-center">
                <div className="text-3xl font-bold text-purple-600">
                  {report.trainingStats?.totalHours || 0}
                </div>
                <div className="text-xs text-gray-500 mt-1">训练时长(h)</div>
              </div>
            </div>
              ...
              <ul className="space-y-2.5">
                {report.improvements && report.improvements.length > 0 ? (
                  report.improvements.map((s, i) => (
                    <li key={i} className="flex items-start gap-2.5 text-gray-700">
                      <span className="w-2 h-2 bg-orange-500 rounded-full mt-1.5 shrink-0"></span>
                      <span>{s}</span>
                    </li>
                  ))
                ) : (
              ...
              {report.goals && report.goals.length > 0 ? (
                report.goals.map((g, i) => (
                  <li key={i} className="flex items-center gap-3 text-gray-700">
                    <span className="w-6 h-6 bg-gradient-to-br from-yellow-400 to-orange-400 text-white rounded-full text-xs flex items-center justify-center font-bold shrink-0">
                      {i + 1}
                    </span>
                    <span>{g}</span>
                  </li>
            ...
            <div className="bg-gray-50 rounded-lg p-4">
              ...
              {report.coachName && (
                <div className="mt-3 pt-3 border-t border-gray-200 text-sm text-gray-500">
                  <span className="font-medium text-gray-600">{report.coachName}</span>
                  {' · '}
                  {report.publishedAt
        ...
        {/* 报告底部信息 */}
        <div className="text-center text-xs text-gray-400 pb-8 print:hidden">
          <p>
