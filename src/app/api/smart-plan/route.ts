...
const skillRecommendations: Record<
  string,
  { skills: string[]; theme: string; activities: string[] }
> = {
  dribbling: {
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // 计算平均分并排序（低分优先）
    const skillScores = Object.entries(skillAverages)
      .map(([skill, data]) => ({
        skill,
    ...
    const planParams = {
      ...
      skillLevel:
        skillScores.reduce((sum, s) => sum + s.avgScore, 0) / skillScores.length >= 7
          ? ('advanced' as const)
    ...
    return NextResponse.json({ success: false, error: '智能推荐失败' }, { status: 500 });
  }
}
