...
export async function GET(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    if (!event) {
      return NextResponse.json({ success: false, error: '比赛事件不存在' }, { status: 404 });
    }
    ...
    return NextResponse.json({ success: false, error: '获取比赛事件详情失败' }, { status: 500 });
  }
}
...
export async function PUT(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    return NextResponse.json({ success: false, error: '更新比赛事件失败' }, { status: 500 });
  }
}
