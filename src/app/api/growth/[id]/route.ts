...
export async function GET(
  ...
  try {
    ...
    const stats = {
      ...
      averageScore: player.assessments.length > 0
        ? Math.round(player.assessments.reduce((sum, a) => sum + (a.overall || 0), 0) / player.assessments.length)
        : 0,
      latestAssessment: player.assessments[0] || null,
      attendanceRate: player.records.length > 0
        ? Math.round((player.records.filter(r => r.attendance === 'present').length / player.records.length) * 100)
        : 0
    }
