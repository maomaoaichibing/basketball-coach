...
export async function POST(request: NextRequest) {
  try {
    ...
    const avgPerformance = trainingRecords.reduce((sum, r) => sum + (r.performance || 0), 0) / (trainingRecords.filter(r => r.performance).length || 1)
    ...
    const overallRating = Math.round(
      (abilities.dribbling + abilities.passing + abilities.shooting +
       abilities.defending + abilities.physical + abilities.tactical) / 6
    )
