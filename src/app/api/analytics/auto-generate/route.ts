...
export async function POST(request: NextRequest) {
  try {
    ...
    if (player.assessments.length > 0) {
      ...
      // 分析弱项（评分 < 6）
      if ((latest.dribbling ?? 0) < 6) { weaknesses.push('运球能力需加强'); trainingSuggestions.push('加强运球基本功训练') }
      if ((latest.shooting ?? 0) < 6) { weaknesses.push('投篮稳定性不足'); trainingSuggestions.push('增加投篮练习次数') }
      if ((latest.passing ?? 0) < 6) { weaknesses.push('传球技术需提升'); trainingSuggestions.push('练习传球准确性和时机把握') }
      if ((latest.defending ?? 0) < 6) { weaknesses.push('防守意识需提高'); trainingSuggestions.push('加强防守脚步和位置感训练') }
      if ((latest.physical ?? 0) < 6) { weaknesses.push('体能需要加强'); trainingSuggestions.push('增加体能训练时间') }
      if ((latest.tactical ?? 0) < 6) { weaknesses.push('战术理解需深化'); trainingSuggestions.push('多观看比赛录像学习战术') }

      // 生成个性化推荐
      const overallScore = Math.round(((latest.dribbling ?? 0) + (latest.shooting ?? 0) + (latest.passing ?? 0) + (latest.defending ?? 0) + (latest.physical ?? 0) + (latest.tactical ?? 0)) / 6)

      // 根据薄弱项生成推荐
      if (weaknesses.length > 0) {
        recommendations.push(`建议加强${weaknesses[0].replace('需要加强', '').replace('需提高', '').replace('不足', '')}训练`)
      }
    ...
    if (latestAssessment) {
      ...
      await prisma.abilityAnalysis.create({
        data: {
          ...
          periodStart,
          periodEnd,
          overallScore: Math.round(((latestAssessment.dribbling ?? 0) + (latestAssessment.shooting ?? 0) + (latestAssessment.passing ?? 0) + (latestAssessment.defending ?? 0) + (latestAssessment.physical ?? 0) + (latestAssessment.tactical ?? 0)) / 6),
