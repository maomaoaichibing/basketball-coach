...
async function exportPlayers() {
  ...
  return players.map((player: any) => ({
    ...
    '状态': player.status === 'training' ? '训练中' : player.status === 'trial' ? '试训' : player.status === 'vacation' ? '休假' : player.status,
...
async function exportRecords() {
  ...
  return records.map((record: any) => ({
    ...
    '出勤状态': record.attendance === 'present' ? '出勤' : record.attendance === 'late' ? '迟到' : '缺勤',
