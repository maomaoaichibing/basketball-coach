...
export async function GET(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    if (!recommendation) {
      return NextResponse.json({ success: false, error: '分班推荐不存在' }, { status: 404 });
    }
    ...
    return NextResponse.json({ success: false, error: '获取分班推荐详情失败' }, { status: 500 });
  }
}
...
export async function PUT(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    return NextResponse.json({ success: false, error: '更新分班推荐失败' }, { status: 500 });
  }
}
...
export async function DELETE(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    return NextResponse.json({ success: false, error: '删除分班推荐失败' }, { status: 500 });
  }
}
