...
export default function OrdersPage() {
  ...
  const [newOrder, setNewOrder] = useState({
    ...
    validFrom: '',
    validUntil: '',
    items: [{ itemType: 'course', courseId: '', name: '', quantity: 1, unitPrice: 0, subtotal: 0, hours: 0, validDays: 365 }],
  })
  ...
  const handleAddItem = () => {
    setNewOrder({
      ...newOrder,
      items: [...newOrder.items, { itemType: 'course', courseId: '', name: '', quantity: 1, unitPrice: 0, subtotal: 0, hours: 0, validDays: 365 }],
    })
  }
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setPayment({ amount: 0, paymentMethod: 'cash', transactionNo: '', operatorName: '', notes: '' })
        fetchOrders()
      }
    } catch (error) {
        ...
        <div className="max-w-7xl mx-auto px-4 py-6">
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
            ...
            <div className="bg-white rounded-lg shadow p-4">
              ...
              <p className="text-2xl font-bold text-yellow-600">{stats.pending + stats.partiallyPaid}</p>
            </div>
            <div className="bg-white rounded-lg shadow p-4">
        ...
        <div className="bg-white rounded-lg shadow overflow-hidden">
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">订单信息</th>
                ...
                <th className="px-4 py-3 text-right text-xs font-medium text-gray-500 uppercase">已付</th>
                <th className="px-4 py-3 text-right text-xs font-medium text-gray-500 uppercase">待付</th>
            ...
            <tbody className="divide-y divide-gray-200">
              {loading ? (
                ...
                <tr>
                  <td colSpan={8} className="px-4 py-8 text-center text-gray-500">暂无订单</td>
                </tr>
              ) : (
                orders.map((order) => (
                  <tr key={order.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3">
                      <div className="text-sm text-gray-900">{order.customerName || order.player?.name || '-'}</div>
                      <div className="text-xs text-gray-500">{order.customerPhone || '-'}</div>
                    </td>
                    <td className="px-4 py-3 text-sm text-gray-500">{getTypeLabel(order.type)}</td>
                    <td className="px-4 py-3 text-sm text-right font-medium">¥{order.totalAmount.toFixed(2)}</td>
                    <td className="px-4 py-3 text-sm text-right text-green-600">¥{order.paidAmount.toFixed(2)}</td>
                    <td className="px-4 py-3 text-sm text-right text-red-600">¥{order.pendingAmount.toFixed(2)}</td>
                  ...
                  {newOrder.items.map((item, index) => (
                    <div key={index} className="flex items-center gap-2 p-3 bg-gray-50 rounded-lg">
                      ...
                      >
                        ...
                        {courses.filter(c => c.type === 'package' || c.type === 'monthly').map((course) => (
            ...
            <div className="p-6 space-y-4">
              <div className="bg-gray-50 p-4 rounded-lg">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="text-red-600 font-bold">¥{selectedOrder.pendingAmount.toFixed(2)}</span>
                </div>
              </div>
