...
export default function AnalyticsPage() {
  const [activeTab, setActiveTab] = useState<'recommendations' | 'analysis' | 'teams'>('recommendations')
              ...
              <div className="space-y-4">
                {recommendations.length === 0 ? (
                  ...
                  recommendations.map((rec) => (
                    <div key={rec.id} className="bg-white rounded-xl p-5 shadow-sm border border-gray-100">
                      <div className="flex items-start justify-between">
                        ...
                        <div className="flex items-center gap-2 ml-4">
                          <span className={`px-2 py-0.5 rounded text-xs ${getStatusColor(rec.status)}`}>
                            {rec.status === 'pending' ? '待处理' : rec.status === 'accepted' ? '已接受' : rec.status === 'rejected' ? '已拒绝' : '已完成'}
                          </span>
                          {rec.status === 'pending' && (
                            <>
                              <button
              ...
              <div className="space-y-4">
                {analyses.length === 0 ? (
                  <div className="bg-white rounded-xl p-8 text-center">
                    ...
                    <p className="text-sm text-gray-400 mt-2">选择学员后点击「生成分析」获取分析报告</p>
                  </div>
                ) : (
                  analyses.map((analysis) => {
                    ...
                    return (
                      <div key={analysis.id} className="bg-white rounded-xl p-5 shadow-sm border border-gray-100">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-3 gap-3 mb-4">
                          {Object.entries(dimensions).map(([key, value]) => (
                            <div key={key} className="bg-gray-50 rounded-lg p-3">
                              ...
                              {progress[key] && (
                                <span className={`text-xs ${(progress[key] as string).startsWith('+') ? 'text-green-600' : 'text-red-600'}`}>
                                  {progress[key]}
                                </span>
                              )}
                            </div>
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex-1">
                          <div className="flex items-center gap-2 mb-2">
                            ...
                            <span className={`px-2 py-0.5 rounded text-xs ${getStatusColor(rec.status)}`}>
                              {rec.status === 'pending' ? '待确认' : rec.status === 'accepted' ? '已接受' : '已拒绝'}
                            </span>
                          </div>
                          <div className="flex items-center gap-4 text-sm">
                            <span className="text-gray-500">当前班级：{rec.currentTeamName || '未分班'}</span>
                            <span className="text-blue-600">→</span>
                            <span className="text-green-600 font-medium">推荐：{rec.recommendedTeamName} ({rec.recommendedGroup})</span>
                          </div>
                          <p className="text-sm text-gray-600 mt-2">{rec.reason}</p>
                          <div className="flex items-center gap-4 mt-2">
                            <span className="text-xs text-gray-400">能力匹配度：{rec.abilityMatch}%</span>
                            <span className="text-xs text-gray-400">推荐时间：{new Date(rec.createdAt).toLocaleDateString()}</span>
                          </div>
                        </div>
                        {rec.status === 'pending' && (
                          <div className="flex items-center gap-2 ml-4">
